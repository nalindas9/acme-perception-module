    ...
    for (it1 = predictions.begin(); it1 < predictions.end(); it1++) {
        ...
        for (row_numbers_it = row_numbers.begin(); 
            ...
            if (confidences > confThreshold) {
                int center_coordinate_x = static_cast<int>(matrix_data[0] * frame.cols);
                int center_coordinate_y = static_cast<int>(matrix_data[1] * frame.rows);
                ...
                nms_boxes.push_back(cv::Rect(leftTop, rightBottom, width, height));
            }
            row = row + 1;
        }
        ...
        for (it2 = indices.begin(); it2 < indices.end(); it2++) {
            ...
            std::cout << "box.x: " << box.x << " box.y: " << box.y << " box.width: " << box.width << " box.height: " << box.height << std::endl;
...
std::vector<std::string> mtp::YoloV3::getOutputLayerNames(const cv::dnn::Net& network) {
